...
export function DataGsmLoginButton({ className, disabled = false, onClick }: DataGsmLoginButtonProps) {
  return <button type="button" className={`data-gsm-login-button ${buttonStyles} ${className ?? ''}`} disabled={disabled} onClick={onClick}>
    <span className="block dark:hidden"><DWhite /></span>
    <span className="hidden dark:block"><DBlack /></span>
    DataGSM으로 로그인
  </button>
}
